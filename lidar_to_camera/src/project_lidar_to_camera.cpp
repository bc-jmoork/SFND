...
void projectLidarToCamera2()
{
    ...
    for(auto it=lidarPoints.begin(); it!=lidarPoints.end(); ++it) {
        ...
        // 3. Once this is done, transform Y back into Euclidean coordinates,
        // and store the result in the variable pt.
        cv::Point pt;
        pt.x = Y.at<double>(0) / Y.at<double>(2);
        pt.y = Y.at<double>(1) / Y.at<double>(2);

        float val = it->x;
        float maxVal = 20.0;
        int red = min(255, (int)(255 * abs((val - maxVal) / maxVal)));
        // int green = min(255, (int)(255 * (1 - abs((val - maxVal) / maxVal))));
        cv::circle(overlay, pt, 5, cv::Scalar(red, red, red), -1);
    }
