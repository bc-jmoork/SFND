...
void detKeypoints1()
{
    ...
    fast_detector = cv::FastFeatureDetector::create(threshold);    
    fast_detector->detect(imgGray, kpts_fast);
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    cout << "FAST features with n= " << kpts_fast.size() << " keypoints in " << 1000 * t / 1.0 << " ms" << endl;

    // visualize results
    visImage = img.clone();
    cv::drawKeypoints(img, kpts_fast, visImage, cv::Scalar::all(-1), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
